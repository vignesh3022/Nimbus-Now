...
const App = () => {
  ...
  const toggleMode=()=>{
    if(mode==='light'){
      ...
      document.getElementById('navbar').style.backgroundColor="#0056b3";
  ...
  useEffect(() => {
    if (Object.keys(datas).length > 0) {
      ...
      setFahrenheit((Math.ceil(datas.main?.temp - 273.15)* 9/5 )+ 32);
  ...
  return (
    <div className="w-full min-h-screen overflow-hidden">
      ...
      <div className="mt-4 mx-2 d-flex flex-column">
        <button
          type="button"
          className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 text-sm rounded-lg px-5 py-2.5 mb-2 focus:outline-none mx-auto font-bold"
          onClick={callbyname}
          id="b1"
        >
          Weather Report
        ...
        <button
          type="button"
          id="b2"
          className="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-blue-300 text-sm rounded-lg px-5 py-2.5 mx-auto mb-2 focus:outline-none font-bold"
  ...
  {!!temp &&
      <div className="block w-9/12 p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 mx-auto mt-5 d-flex flex-column justify-center items-center sm:w-6/12 md:w-4/12 lg:w-3/12 mb-5">
        <p className="d-flex flex-row font-bold">
          <BsThermometerHalf size={30} className="me-3 text-red-500 " /> {temp} °C
        </p>
        <p className="d-flex flex-row font-bold">
          <BsThermometerHalf size={30} className="me-3 text-red-500" /> {fahrenheit} °F
        </p>
        <p className="d-flex flex-row font-bold">
          <IoIosWater size={30} className="me-3 text-blue-500" /> {humidity} %
        </p>
        <p className="d-flex flex-row font-bold">
          <FaSun className="me-3 text-orange-300" size={30} /> {description}
        </p>
        <p className="d-flex flex-row font-bold">
          <BiMap className="me-3 text-green-500" size={30} /> {name}
        </p>
        <p className="d-flex flex-row font-bold">
          <BsWind className="me-3 text-blue-300" size={30} /> {windspeed}Km/h
        </p>
      </div>
  }
      <ToastContainer/>
